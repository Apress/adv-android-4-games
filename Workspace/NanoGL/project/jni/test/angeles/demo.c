...
static float fixedToFloat(long value)
{
    return (float)(value/65536.0);
}

#define FIXED(value) floatToFixed(value)
#define FLOAT(value) fixedToFloat(value)
/*
#define GL_FIXED                          0x140C

void glScalex(long X, long Y, long Z) {
	glScalef(FLOAT(X),FLOAT(Y), FLOAT(Z) );
}
void glTranslatex(long X, long Y, long Z) {
	glTranslatef(FLOAT(X),FLOAT(Y), FLOAT(Z) );
}
void glRotatex(long X, long Y, long Z, long P) {
	glRotatef(FLOAT(X),FLOAT(Y), FLOAT(Z), FLOAT(P));
}
void glFrustumx(long X, long Y, long Z, long P,  long Q, long R) {
	glFrustum(FLOAT(X),FLOAT(Y), FLOAT(Z), FLOAT(P), FLOAT(Q), FLOAT(R));
}
*/
...
static void drawModels(float zScale)
{
	...
	/*
            glTranslatef((x * translationScale) ,
                         (y * translationScale) ,
                         0);
            glRotatef ((randomUInt() % 360) , 0, 0, 1.0);
